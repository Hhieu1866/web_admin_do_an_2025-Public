...
export async function ceredntialLogin(formData) {
    try {
        ...
        if (response.error) {
            ...
            if (response.error.includes("User not found") || response.error.includes("không tìm thấy")) {
                return {
        ...
        if (errorMessage.includes("User not found") || errorMessage.includes("không tìm thấy") || errorMessage.includes("not exist")) {
            return {
                error: "Tài khoản không tồn tại"
            };
        }
        ...
        return {
            error: "Đăng nhập thất bại. Vui lòng kiểm tra thông tin đăng nhập của bạn."
        };
    }
}